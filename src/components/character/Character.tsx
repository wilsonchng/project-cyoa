  ...
  return (
    <div className="container">
      {Object.values(Ability).map((ability) => {
        return (
          <span className="ability" key={ability} onClick={() => setOpen(true)}>
            {`${ability}: ${character.ability[ability]}`}
          </span>
        );
      })}
      <Modal header="Ability Info" open={open} onClose={() => setOpen(false)}>
        <AbilityDescription />
      </Modal>
    </div>
  );
...
const AbilityDescription = () => {
  return (
    <div className="container">
      ...
      <p>
        <strong>FITNESS</strong> determines a character's overall physical
        abilities, such as how fast they can run, how long they can exert
        themselves before getting tired, and reflexes
      </p>
      <p>
        <strong>FIREARMS</strong> determines a character's skill with guns,
        affecting accuracy, reload speed and knowledge of guns
      </p>
      <p>
      ...
      <p>
        <strong>MEDICINE</strong> determines a character's ability to evaluate
        the severity of injuries and sicknesses, whether they know the
        appropriate medical actions to take, and recognising states of decay
      </p>
      <p>
        <strong>SURVIVAL</strong> determines a character's ability to survive in
        the wilderness, such as finding shelter, starting a fire, finding food
        and water, distinguishing edible plants from poisonous ones
      </p>
    </div>
  );
};
